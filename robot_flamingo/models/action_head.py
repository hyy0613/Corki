...
class DeterministicDecoder(ActionDecoder):
    def __init__(
        ...
        if use_episode:
            self.rank = rank  
            self.episode_x = MLPTanhHead(hidden_size, self.rank)
            self.episode_y = MLPTanhHead(hidden_size, self.rank)
            self.episode_z = MLPTanhHead(hidden_size, self.rank)
            self.episode_euler_x = MLPTanhHead(hidden_size, self.rank)
            self.episode_euler_y = MLPTanhHead(hidden_size, self.rank)
            self.episode_euler_z = MLPTanhHead(hidden_size, self.rank)
            self.gripper = MLPSigmoidHead(hidden_size, 1 * multi_step_action)
            self.episode_loss = episode_loss
        ...
        if not use_diff and not use_episode:
        ...
        if self.mask_ratio > 0:
            temp_features = in_features
            if self.use_vit:
                temp_features = in_features // 2
            self.word_embeddings = nn.Embedding(1, temp_features)
    ...
    def forward(  # type: ignore
        ...
        if not hidden_inference:
            ...
            if self.mask_ratio > 0 and self.training:
                random_matrix = torch.rand((*input_feature.shape[:2], 1), device=input_feature.device)
                bs, sq = input_feature.shape[:2]
                de = input_feature.device
                if self.use_waypoint:
                    random_matrix = torch.ones_like(input_feature[:, :, :1])

                    if self.adaptive:
                        self.use_gt = np.random.random() > 0.5

                    waypoint_mask = is_waypoint.byte().unsqueeze(-1)[:bs, :sq, :1]
                    random_matrix = random_matrix * waypoint_mask
                else:
                    random_matrix = torch.tensor([1] + [0] * (take_steps - 1), device=de).repeat(bs, sq // take_steps + 1)[:bs, :sq].view(bs, sq, 1)
                if self.use_gt:
                    input_feature = torch.where(random_matrix > self.mask_ratio, input_feature,
                                                self.word_embeddings.weight[None])

            if self.use_vit:
                vit_shape = input_feature.shape
                vit_feature = vit_feature.view(*vit_shape)
                input_feature = torch.cat([input_feature, vit_feature], dim=-1)
            ...
            input_feature = self.word_embeddings.weight[None]
        ...
        if not isinstance(self.rnn, nn.Sequential) and isinstance(self.rnn, nn.RNNBase):
            ...
            if input_feature.shape[1] == 1:
                ...
                if self.adaptive and not (self.use_waypoint and self.use_gt):
                    self.hidden_state = h_0
                    pos_out = []
                    gripper_out = []
                    bs, sq, latent = input_feature.shape
                    input_idx = 0  # 表示当前输入为第几帧
                    frame_tensor = torch.tensor([0] * bs).cuda().reshape(bs, 1)
                    while (input_idx < sq):
                        idx_tensor = torch.tensor([input_idx] * bs).cuda().reshape(bs, 1)
                        idx_tensor = idx_tensor >= frame_tensor
                        input_frame = torch.where(idx_tensor.repeat(1, latent).unsqueeze(1),
                                                  input_feature[:, input_idx].unsqueeze(1),
                                                  self.word_embeddings.weight[None].repeat(6, 1, 1))
                        x, h_n = self.rnn(input_frame, self.hidden_state)
                        if self.use_episode:
                            par_dict = {}
                            parname_list = ['x_par', 'y_par', 'z_par', 'x_eul', 'y_eul', 'z_eul']
                            par_dict['x_par'] = self.episode_x(x)
                            par_dict['y_par'] = self.episode_y(x)
                            par_dict['z_par'] = self.episode_z(x)
                            par_dict['x_eul'] = self.episode_euler_x(x)
                            par_dict['y_eul'] = self.episode_euler_y(x)
                            par_dict['z_eul'] = self.episode_euler_z(x)
                            gripper = self.gripper(x)
                            gripper_out.append(gripper.squeeze(1))  # 夹爪开闭

                            if "point" in self.episode_loss:
                                bs, seq_len = par_dict['x_par'].shape[:2]
                                for parname in parname_list:
                                    par_dict[parname] = par_dict[parname].view(bs * seq_len, -1)
                                time_var = torch.arange(self.multi_step_action).cuda().float()
                                time_matrix = torch.cat([time_var ** r for r in range(self.rank)], dim=0).reshape(
                                    self.rank, self.multi_step_action)
                                for parname in parname_list:
                                    par_dict[parname] = torch.mm(par_dict[parname], time_matrix).view(bs, seq_len, -1)

                                actions = torch.stack([par_dict[par] for par in parname_list], dim=-1)
                                pos_out.append(actions.squeeze(1))
                        else:
                        ...
                        step_tensor = get_waypoint_index(actions[:, -1].detach(),threshold) # 此处输出的action是已经累加过的，不应该再做cumsum
                        frame_tensor = frame_tensor + torch.where(idx_tensor, step_tensor, 0)
                        input_idx = input_idx + 1
                    pos_out_tensor = torch.stack(pos_out, dim=1)
                    gripper_out_tensor = torch.stack(gripper_out, dim=1)
                    return pos_out_tensor, gripper_out_tensor
                else:
                    self.hidden_state = h_0
                    x, h_n = self.rnn(input_feature, self.hidden_state)
                    self.hidden_state = h_n
        ...
        if self.use_episode:
            ...
            if "point" in self.episode_loss:
                ...
                for parname in parname_list:
                    par_dict[parname] = par_dict[parname].view(bs * seq_len, -1)
                time_var = torch.arange(self.multi_step_action).cuda().float()
                time_matrix = torch.cat([time_var ** r for r in range(self.rank)], dim=0).reshape(self.rank,
                                                                                                  self.multi_step_action)
                for parname in parname_list:
                    par_dict[parname] = torch.mm(par_dict[parname], time_matrix).view(bs, seq_len, -1)
                ...
                return actions, gripper
            elif "param" in self.episode_loss:
                return par_dict, gripper
            else:
                raise NotImplementedError("episode loss is not implemented yet")
...
class GPTDecoderActPad(ActionDecoder):
    ...
    def forward(self, input_feature: torch.Tensor, rgb=None):
        ...
        input_feature = input_feature.view(-1, self.window_size, self.global_latent,
                                           input_feature.shape[-1])  # bs, seq_len, n_tok, feat_dim
...
def get_waypoint_index(actions,threshold):
    ...
    for i in range(actions.shape[0]):
        ...
        while ith < len(action):
            ...
            # action[..., :6] = np.cumsum(action[..., :6], axis=0) 累加过的action不能做cumsum
            action[..., 6] = action[..., 6]
            ...
            ith += keyframe_inds[0]
            step_list.append(ith)
            break
    return torch.tensor(step_list).cuda().reshape(actions.shape[0], 1)
