...
class MPTFlamingo(nn.Module):
    def __init__(
        ...
        if self.use_vit:
            self.num_latents = 1
            self.perceiver_feature = PerceiverResampler(dim = self.vis_dim,num_latents=self.num_latents)
        ...
        if decoder_type == 'lstm':
            lm_head = DeterministicDecoder(in_features, self.window_size,
                                           ...
                                           use_episode=use_episode, episode_loss=episode_loss, mask_ratio=mask_ratio,
                                           skip_steps=skip_steps,use_waypoint=use_waypoint,use_vit=use_vit,rank=rank,rlbench=rlbench,adaptive=adaptive)
    ...
    def forward(
        ...
        if not hidden_inference:
                ...
                if self.use_hist:
                    ...
                    if not self.use_gripper or self.fusion_mode == 'two_way':
                        ...
                        if self.fusion_mode == 'pre':
                            ...
                            vit_feature = self._encode_multi_vision_post_fusion(vision_x, vision_gripper)
            ...
            output_hs = None
            if self.fusion_mode == 'post' and self.use_vit:
                vit_feature = self._encode_multi_vision_post_fusion(vision_x, vision_gripper)
        if self.use_vit:
            output_hs = self.lm_head(output_hs, state_tensor=state_tensor, return_feature=return_feature,hidden_inference=hidden_inference,vit_feature=vit_feature)
        elif self.use_waypoint:
            output_hs = self.lm_head(output_hs, state_tensor=state_tensor, return_feature=return_feature,hidden_inference=hidden_inference,is_waypoint=is_waypoint,threshold=self.threshold)
        else:
            output_hs = self.lm_head(output_hs, state_tensor=state_tensor, return_feature=return_feature,hidden_inference=hidden_inference,take_steps=self.take_steps)
    ...
    def _encode_multi_vision_pre_fusion(self, vision_rgb: torch.Tensor, vision_gripper: torch.Tensor,
                                        state_tensor=None):
        """
        Compute media tokens from vision input by passing it through vision encoder and conditioning language model.
        Args:
    ...
    def _encode_multi_vision_post_fusion(self, vision_rgb: torch.Tensor, vision_gripper: torch.Tensor,
        ...
        vision_rgb = self._encode_vision(vision_rgb)
        vision_gripper = self._encode_vision(vision_gripper)
        vit_rgb, vit_gripper = vision_rgb.clone(), vision_gripper.clone()
        ...
        if self.use_vit:
            vit_rgb,vit_gripper = self.perceiver_feature(vit_rgb),self.perceiver_feature(vit_gripper)
            vit_feature = torch.cat([vit_rgb,vit_gripper],dim=2)
            return vit_feature
    ...
    def _encode_history_vision_post_fusion(self, vision_rgb: torch.Tensor, vision_gripper: torch.Tensor,
                                           state_tensor=None):
        """
        Compute media tokens from vision input by passing it through vision encoder and conditioning language model.
        Args:
