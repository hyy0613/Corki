...
def get_ckpt_name(args, epoch=-1):
    if args.use_gripper:
        ckpt_name = 'checkpoint_gripper_{}_hist_{}_{}'.format(args.fusion_mode, args.hist_window,
                                                              '' if not args.sep_resampler else 'sep_')
    else:
        ckpt_name = 'checkpoint_no_gripper_hist_{}_{}'.format(args.hist_window,
                                                              '' if not args.sep_resampler else 'sep_')
    if args.real_data:
        ckpt_name += 'real_'
...
def train_one_epoch_calvin(
    ...
    for num_steps, batch_calvin in t:
        ...
        # do the same to the attention mask
        if args.fusion_mode != 'vit_concat':
            attention_mask = batch_calvin[1][1].to(device_id, non_blocking=True).unsqueeze(1).repeat(1, images.shape[1],
                                                                                                     1)
        else:
        ...
        if args.use_waypoint:
            is_waypoint = batch_calvin[6].to(device_id, dtype=cast_dtype, non_blocking=True)
        else:
            is_waypoint = None
        # merge the batch and the sequence dimension
        images = images.flatten(0, 1)
        ...
        def tensor_select(t, d=0):
            return torch.index_select(t, dim=d, index=torch.arange(0, t.size(d), args.skip_steps).cuda())

        if args.skip_steps > 1:
            images = tensor_select(images)
            gripper = tensor_select(gripper)
            input_ids = tensor_select(input_ids)
            attention_mask = tensor_select(attention_mask)
            labels[0] = tensor_select(labels[0], d=1)
            labels[1] = tensor_select(labels[1], d=1)
        ...
        # compute loss
        if not args.use_episode:  # yiyang-modify
            ...
            if "sum" in args.episode_loss:
                labels[0] = torch.cumsum(labels[0], -2)  # 将action沿着倒数第二维做累加
            if "point" in args.episode_loss:
                num_actions, bin_actions = output.logits[0], output.logits[1]

                loss_calvin_num = torch.nn.functional.huber_loss(num_actions, labels[0])
                loss_calvin_bin = torch.nn.functional.binary_cross_entropy(bin_actions.unsqueeze(-1), labels[1])
                loss_calvin = loss_calvin_num + loss_calvin_bin * 0.01
            elif "param" in args.episode_loss:
                par_dict, bin_actions = output.logits[0], output.logits[1]
                parname_list = ['x_par', 'y_par', 'z_par', 'x_eul', 'y_eul', 'z_eul']
                par_tensor = torch.stack([par_dict[par_name] for par_name in parname_list],
                                         dim=-2).cuda().float()  # 参数矩阵，用于计算loss

                actions = torch.transpose(labels[0], dim0=-1, dim1=-2)
                bs, sq, ac = actions.shape[0], actions.shape[1], actions.shape[2]
                actions = actions.reshape(bs * sq * ac, -1).cpu().numpy()
                time_var = np.array(range(5))
                params = np.zeros((bs * sq * ac, 4))  # 实际loss计算参数矩阵
                for i in range(bs * sq * ac):
                    coefficients = np.polyfit(time_var, actions[i], 3)
                    params[i, :] = coefficients
                params = torch.tensor(params).reshape(bs, sq, ac, -1).cuda().float()

                loss_calvin_param = torch.nn.functional.huber_loss(par_tensor, params)
                loss_calvin_bin = torch.nn.functional.binary_cross_entropy(bin_actions.unsqueeze(-1), labels[1])
                loss_calvin = loss_calvin_param * 10 + loss_calvin_bin * 0.01
        ...
        # Log loss to console
        if ((num_steps + 1) % args.logging_steps == 0) and args.rank == 0:
            if "param" in args.episode_loss:
                print(
                    f"Step {num_steps + 1}/{num_batches_per_epoch} of epoch {epoch + 1}/{args.num_epochs} complete. Loss: (all){loss_calvin.item():.3f} (mse){loss_calvin_param.item():.3f} (bce){loss_calvin_bin.item():.3f}"
                )
            else:
                print(
        ...
        if "param" in args.episode_loss:
            t.set_postfix({"avg loss": sum(mv_avg_loss[-avg_horizon:]) / avg_horizon, "loss": loss_calvin.item(),
                           "Lparam": loss_calvin_param.item(), "Lbin": loss_calvin_bin.item()})
        else:
            t.set_postfix({"avg loss": sum(mv_avg_loss[-avg_horizon:]) / avg_horizon, "loss": loss_calvin.item(),
                           "Lnum": loss_calvin_num.item(), "Lbin": loss_calvin_bin.item()})
...
def train_one_epoch_calvin_cotrain(
    ...
    for num_steps, (batch_coco, batch_vqa, batch_calvin) in t:
        ...
        avg_horizon = min(100, len(mv_avg_loss))
        t.set_postfix({"avg calvin loss": sum(mv_avg_loss[-avg_horizon:]) / avg_horizon,
                       "avg coco loss": sum(mv_avg_loss_coco[-avg_horizon:]) / avg_horizon,
                       "avg vqa loss": sum(mv_avg_loss_vqa[-avg_horizon:]) / avg_horizon,
...
def train_one_epoch_calvin_two_way(
    ...
    for num_steps, batch_calvin in t:
        ...
        state_tensor = batch_calvin[4].to(device_id, dtype=cast_dtype, non_blocking=True).repeat(2, 1, 1).unsqueeze(
            2).unsqueeze(2)
        # import pdb; pdb.set_trace()
        # merge the batch and the sequence dimension
        # images = images.flatten(0, 1)
