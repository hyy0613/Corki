...
def main():
    ...
    model_awe,image_processor_awe,tokenizer_awe = create_model_and_transforms(
        args.vision_encoder_path,
        args.vision_encoder_pretrained,
        args.lm_path,
        args.tokenizer_path if args.tokenizer_path else args.lm_path,
    ...
    model_awe.load_state_dict(torch.load(checkpoint_path), strict=False)

    if args.sep_lm_head:
        model.lm_head.requires_grad_(True)
        model_awe.lm_head.requires_grad_(True)
    else:
        model.lang_encoder.lm_head.requires_grad_(True)
        model_awe.lang_encoder.lm_head.requires_grad_(True)

    device_id = args.rank % torch.cuda.device_count()
    if args.precision == "bf16" or args.precision == "amp_bfloat16" or args.precision == "amp_bf16":
        model = model.bfloat16()
        model_awe = model_awe.bfloat16()
    elif args.precision == "fp16":
        model = model.half()
        model_awe = model_awe.half()
    else:
        model = model.float()
        model_awe = model_awe.float()
    model = model.to(device_id)
    model_awe = model_awe.to(device_id)
    model.eval()
    model_awe.eval()

    ddp_model = DDP(model, device_ids=[device_id])
    ddp_model_awe = DDP(model_awe,device_ids=[device_id])
    if args.residual:
    ...
    awe_checkpoint = torch.load(
        "/mnt/dolphinfs/hdd_pool/docker/user/hadoop-vacv/huangyiyang02/new/RoboFlamingo-origin/exps/robot_flamingo_awe/run9/checkpoint_gripper_post_hist_1_aug_10_4_traj_cons_ws_12_mpt_dolly_3b_8.pth8.pth",map_location="cpu")
    ddp_model.load_state_dict(checkpoint["model_state_dict"], False)  # 只保存了求梯度的部分
    ddp_model_awe.load_state_dict(awe_checkpoint["model_state_dict"], False)
    ddp_model.eval()
    ddp_model_awe.eval()
    eval_log_dir = None
    # if args.visualize:
